...
export default function BBGameBoard({
  ...
  const remaining = BB_MAX_GUESSES - guesses.length;
  ...
  function submitGuess(c) {
    ...
    setError("");
    const result     = evaluateBBGuess(c, answer);
    ...
    const didFail = !didWin && newGuesses.length >= BB_MAX_GUESSES;
    if (!didWin && !didFail) {
  ...
  function handleShare() {
    const label = mode === "bb_unlimited"
      ? "Big Brotherdle Unlimited"
      : mode === "bb_archive"
      ? `Big Brotherdle Archive #${puzzleNum}`
      : `Big Brotherdle #${puzzleNum}`;
    const shareUrl = mode === "bb_unlimited"
      ? "bigbrotherdle.com/bb/unlimited"
      : mode === "bb_archive"
      ? "bigbrotherdle.com/bb/archive"
      : "Bigbrotherdle.com/bb";
    const hintLines = [];
    ...
    const text = `${label} — ${won ? guesses.length : "X"}/${BB_MAX_GUESSES} 👁${hintBlock}\n`
  ...
  const bannerLabel = mode === "bb_unlimited"
    ? "Unlimited"
    : mode === "bb_archive"
    ? `Archive #${puzzleNum}`
    : `Big Brotherdle #${puzzleNum}`;
  const bannerIcon = mode === "bb_unlimited" ? "♾️" : mode === "bb_archive" ? "🗓️" : "👁";
    ...
    <div>
        ...
        <div className="input-area">
          <div className="search-wrap">
            <input
              ...
              placeholder="Search by houseguest name…"
              value={query}
              autoComplete="off"
          ...
          <div className="search-note">You are guessing a houseguest and their specific season appearance</div>
      ...
      {guesses.length > 0 && (
        <div className="hint-bar">
          ...
          >
            {hintNeighbors ? "✓ Neighbors Revealed" : "💡 Reveal Evicted Neighbors"}
          </button>
          {!gameOver && !won && (
      ...
      {(hintEpisode || hintNeighbors) && answer && (() => {
        ...
        const outcomeText = isWinner
          ? "Made it to Finale Night (Won)"
          : isFinalist
          ? "Made it to Finale Night"
          : `Evicted during Week ${answer.weekOut ?? "?"}, Day ${answer.day ?? "?"}`;
        return (
          <div className="hint-panels">
            ...
            {hintNeighbors && (
              <div className="hint-panel">
                <div className="hint-panel-label">Evicted Between</div>
      ...
      {gameOver && (
        <div className={`status-banner ${won ? "win" : "lose"}`}>
          {won
            ...
            : gaveUp
            ? <>{bannerLabel} — you gave up. {mode !== "bb" ? "Try another!" : "Better luck tomorrow."}</>
      ...
      <div className="col-headers">
        <div className="col-head">Houseguest</div>
        {BB_COLUMNS.map(c => (
