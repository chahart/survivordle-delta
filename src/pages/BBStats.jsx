...
import { fetchBBDailyStats, fetchBBGlobalStats } from "../shared/supabase";
import {
  loadBBStats, loadBBUnlimitedStats, loadBBSandwichStats, loadBBSandwichUnlimitedStats,
} from "../shared/storage";
...
const BB_HEADER_CSS = `
  ...
  /* Recolor Survivordle-orange brand accents to BB blue on this page.
     Semantic colors (correct/close/wrong, difficulty easy/hard) are left untouched. */
  .bb-theme .ul-tab.active,
  .bb-theme .sp-section-title,
  ...
  .bb-theme .sp-fg-bar {
    background: linear-gradient(90deg, #1a6fbf99, #5aaedd99);
  }
`;
...
function difficultyLabel(avg, sampleOk, easyThreshold, hardThreshold) {
  ...
  return                            { label: "Typical difficulty", color: "#5aaedd", emoji: "👁" };
}
...
function MyStatsTab() {
  ...
  const unlim    = loadBBUnlimitedStats();
  const swDaily  = loadBBSandwichStats();
  const swUnlim  = loadBBSandwichUnlimitedStats();
  const dStats   = daily || { played: 0, wins: 0, currentStreak: 0, maxStreak: 0, dist: {} };
...
// ── Main BB Stats Page ─────────────────────────────────────────────────────────
export default function BBStats() {
  ...
  function loadDaily() {
    ...
    fetchBBDailyStats().then(d  => { setDailyData(d);  setLoadingDaily(false);  });
  }
  function loadGlobal() {
    setLoadingGlobal(true);
    fetchBBGlobalStats().then(d => { setGlobalData(d); setLoadingGlobal(false); });
  }

  useEffect(() => { loadDaily(); loadGlobal(); }, []);

  useSEO({
    title: "Stats | Big Brotherdle",
    description: "Big Brotherdle statistics: your personal stats, today's difficulty rating, and global player data.",
    canonical: "https://survivordle.com/bb/stats",
  });
